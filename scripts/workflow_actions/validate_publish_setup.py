...
def validate_wally_ready() -> bool:
    ...
    3. Reads and prints the package ``name`` and ``version`` from wally.toml
       so the workflow log shows which package is being used as the probe.

    Note: this function intentionally does **not** create ``default.project.json``.
    That file is only required when actually calling ``wally publish`` or
    ``wally login`` (handled separately in publish.py and the publish action).
    Creating it here would leave a new untracked file in the working tree that
    publish_detect_changed_packages would then treat as a package change.
