...
function ClientComm.new(parent: Instance, usePromise: boolean, namespace: string?)
	...
	if folder == nil then
		error(`Timed out after {Util.WaitForChildTimeout} seconds. Could not find namespace '{namespace}' for ClientComm in parent: {parent:GetFullName()}`)
	end
...
--[=[
    Destroys the ClientComm object. Sets the objects to be a table that errors if you attempt to access any property.
]=]
function ClientComm:Destroy()
    local InstanceName = if self._instancesFolder then self._instancesFolder:GetFullName() else "UNKNOWN"

    for i = #self._objects, 1, -1 do
        local object = self._objects[i]
        for key, value in pairs(object) do
            if typeof(value) == "table" and value.Destroy then
                value:Destroy()
            end
            object[key] = setmetatable({}, {__index = function(t, k)
                error(("Cannot access property '%s.%s' on destroyed ClientComm for '%s'"):format(key, tostring(k), InstanceName), 2)
            end})
        end
        self._objects[i] = nil
    end
